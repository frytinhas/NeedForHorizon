...
UStreamlineDLSSGSupport UStreamlineLibraryDLSSG::DLSSGSupport = UStreamlineDLSSGSupport::NotSupportedByPlatformAtBuildTime;
#if WITH_STREAMLINE
int32 UStreamlineLibraryDLSSG::MinDLSSGDriverVersionMajor = 0;
int32 UStreamlineLibraryDLSSG::MinDLSSGDriverVersionMinor = 0;
...
void UStreamlineLibraryDLSSG::GetDLSSOnScreenMessages(TMultiMap<FCoreDelegates::EOnScreenMessageSeverity, FText>& OutMessages)
{
	...
	// We need a valid DLSSSupport, so calling this here in case other UStreamlineLibraryDLSSG functions which call TryInitDLSSGLibrary() haven't been called
...
bool UStreamlineLibraryDLSSG::IsDLSSGSupported()
{
	...
	return QueryDLSSGSupport() == UStreamlineDLSSGSupport::Supported;
#else
	return false;
#endif
}
...
UStreamlineDLSSGSupport UStreamlineLibraryDLSSG::QueryDLSSGSupport()
{
	...
	if (!TryInitDLSSGLibrary())
	{
		...
		return UStreamlineDLSSGSupport::NotSupported;
	}
#endif
	return DLSSGSupport;
}
...
void UStreamlineLibraryDLSSG::GetDLSSGMinimumDriverVersion(int32& MinDriverVersionMajor, int32& MinDriverVersionMinor)
{
	...
	if (!TryInitDLSSGLibrary())
	{
		UE_LOG(LogStreamlineBlueprint, Error, TEXT("GetDLSSMinimumDriverVersion should not be called before PostEngineInit"));
	}
	MinDriverVersionMajor = MinDLSSGDriverVersionMajor;
	MinDriverVersionMinor = MinDLSSGDriverVersionMinor;
#else
...
bool UStreamlineLibraryDLSSG::TryInitDLSSGLibrary()
{
	...
	{
		DLSSGSupport = UStreamlineDLSSGSupport::Supported;
	}
	else
	{
		DLSSGSupport = UStreamlineDLSSGSupport::NotSupported;
	}
...
void UStreamlineLibraryDLSSG::Startup()
{
	...
	UStreamlineLibraryDLSSG::DLSSGSupport = UStreamlineDLSSGSupport::NotSupportedByPlatformAtBuildTime;
#endif
}
