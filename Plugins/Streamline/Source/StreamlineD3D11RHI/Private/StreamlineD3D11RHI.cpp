...
public:
	...
	TCHAR* GetName() const override final { return TEXT("FStreamlineD3D11DXGISwapchainProvider"); }
...
#else
				// DLSS-FG disabled for D3D11 for now
				UE_LOG(LogStreamlineD3D11RHI, Log, TEXT("Skip registering IDXGISwapchainProvider, DLSS-FG not supported with D3D11"));
#endif
			}
			else
			{
...
void FStreamlineD3D11RHIModule::StartupModule()
{
	...
	if(FApp::CanEverRender())
	{
		if ((GDynamicRHI != nullptr) && (GDynamicRHI->GetName() == FString("D3D11")))
		{
			...
			{
				StreamlineRHIModule.InitializeStreamline(false);
				if (IsStreamlineSupported())
				{
