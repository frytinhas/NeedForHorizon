...
void FStreamlineRHI::SetStreamlineData(FRHICommandList& CmdList, const FRHIStreamlineArguments& InArguments)
{
	...
	StreamlineConstants.cameraViewToClip = ToSL(InArguments.CameraViewToClip);
	...
	StreamlineConstants.cameraFOV = InArguments.CameraFOV;
...
STREAMLINERHI_API void PlatformCreateStreamlineRHI()
{
...

	{
		...
		GStreamlineSupport = (bIsDX11 || bIsDX12) ? EStreamlineSupport::Supported : EStreamlineSupport::NotSupported;
		...
		if (GStreamlineSupport == EStreamlineSupport::Supported)
		{
			...
			if (IsStreamlineSupported())
			{
				...
				UE_LOG(LogStreamlineRHI, Log, TEXT("NGXBinariesDir %s"), *NGXBinariesDir);
...
static bool ShouldLoadDebugOverlay()
{
	...
	{
		UE_LOG(LogStreamlineRHI, Log, TEXT("Loading Streamline debug overlay (sl.imgui) due to -sldebugoverlay command line, which has priority over the config file setting of %u"), bLoadDebugOverlay);
		bLoadDebugOverlay = true;
	}
...
void FStreamlineRHIModule::InitializeStreamline(bool bAllowDLSSG)
{
...
#define NGX_DLSS_BINARY_NAME (TEXT("nvngx_dlss.dll"))
	...
	UE_LOG(LogStreamlineRHI, Log, TEXT("sl::Preferences::featuresToLoad = {%s}" ),
	...
	{
		UE_LOG(LogStreamlineRHI, Error, TEXT("Failed to initialize Streamline (%s)"), ANSI_TO_TCHAR(sl::getResultAsStr(Result)));
